...
When the macOS **Swipe between pages** Trackpad gesture is enabled, a
two-finger swipe right over the expanded nook invokes that same back path.
OpenNook starts tracking only while a back destination exists and only for a
horizontal gesture, so ordinary vertical scrolling continues unchanged. The
leading glyph remains the visible, keyboard-accessible alternative; the swipe
is an accelerator, not the only way back.
